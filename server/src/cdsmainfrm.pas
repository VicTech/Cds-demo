...
type
  ...
  public
    ...
    procedure CdsTermTimeControlResult(m_FrameHead: TCDSTermSendCommFrameHead;
      m_RecvBuf: pchar; m_BufLenth: integer; m_Iocpsocket: TRPClientContext);
    procedure CdsTermOrderExecute(m_FrameHead: TCDSTermSendCommFrameHead;
      m_RecvBuf: pchar; m_BufLenth: integer; m_Iocpsocket: TRPClientContext);
    procedure CdsTermGetPatientInfo(m_FrameHead: TCDSTermSendCommFrameHead;
      m_RecvBuf: pchar; m_BufLenth: integer; m_Iocpsocket:
      TRPClientContext);
...
begin
  ...
  m_PChrCDSPatientInfoData := AllocMem(CDSPatientMaxNum * SizeOf(
    TCDSPatientInfo));
  m_PChrCDSTermEmployeeData := AllocMem(CDSUserMaxNum * SizeOf(
    TCDSTermEmployee));
  m_PChrCDSClientEmployeeData := AllocMem(CDSTermMaxNum * SizeOf(
    TCDSClientEmployee));
...
begin
  ...
  freemem(m_PChrCDSPatientInfoData, CDSPatientMaxNum * SizeOf(TCDSPatientInfo));
  freemem(m_PChrCDSTermEmployeeData, CDSUserMaxNum * SizeOf(TCDSTermEmployee));
  freemem(m_PChrCDSClientEmployeeData, CDSTermMaxNum * SizeOf(TCDSClientEmployee));
...
begin
  ...
  {----------------------------------------------------------------------------}
    ...
    begin
      ...
      {----------------------------------------------------------------------------}
    if m_FrameHead.m_skType = skOrderExecute then //终端医嘱执行
    begin
      CdsTermOrderExecute(m_FrameHead, m_RecvBuf, m_BufLenth, m_Iocpsocket);
      exit;
    end;

    {----------------------------------------------------------------------------}
    if m_FrameHead.m_skType = skGetPatientInfo then //终端医嘱执行
    begin
      CdsTermGetPatientInfo(m_FrameHead, m_RecvBuf, m_BufLenth, m_Iocpsocket);
      exit;
    end;
...
begin
  ...
  try
    ...
    m_CDSClientEmployeeData := Pointer(m_PChrCDSClientEmployeeData);
...
begin
  ...
  try
    ...
    m_CDSTermEmployeeData := Pointer(m_PChrCDSTermEmployeeData);
    while not m_ADODataSet.Eof do
    begin
...
begin
  ...
  try
    ...
    m_CDSPatientInfoData := Pointer(m_PChrCDSPatientInfoData);
    ...
    begin
      ...
      m_ADODataSet.Next;
      Inc(m_CDSPatientInfoData);
    end;
  finally
    g_ThrdlstPatientsInfo.UnLockList;
 //  freemem(m_PBuffers,(m_ADODataSet.RecordCount)*SizeOf(TCDSPatientInfo));
    m_ADODataSet.Close;
    m_ADODataSet.Free;
    m_ADOConnectionServer.Close;
  end;
...
begin
  ...
  try
    for m_IntiLoop := 0 to count - 1 do
    begin
      if PCDSTermEmployee(Items[m_IntiLoop]).m_IocpSocket <> nil then
      begin
        if PCDSTermEmployee(Items[m_IntiLoop]).m_IocpSocket.m_Socket = m_IocpSocket.m_Socket then
        begin
...
var
  ...
  m_SendBuffer: array[1..256] of Byte;
  m_CRCCheck: byte;
  m_Login, m_BlCheck: boolean;
  m_IntiLoop: integer;
begin
  m_BlCheck := true;
  ...
  begin
    ...
    if (m_CRCCheck <> CalCrc(m_RecvBuf, m_BufLenth - 1)) then
    begin
  ...
  begin
    ...
    try
      for m_IntiLoop := 0 to count - 1 do
      begin
        m_TempTerm := Items[m_IntiLoop];
        if m_TempTerm.m_StrTermNum = inttostr(m_FrameHead.m_CarTermNum) then
        begin
  ...
  m_CRCCheck := CalCrc(@m_SendBuffer[1], TermRecvFrameHeadLth);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        m_CRCCheck := CalCrc(@m_SendBuffer[1], ClientSendFrameHeadLth + SizeOf(m_ClientRecvCallDtIf));
...
begin
  ...
  begin
    ...
    begin
      //寻找病人及用户
      m_TempClient := FindClientByUserNum(DoubleToHexStr(m_TermCheckCard.m_CardUserNum));
   ...
   //   m_TermCheckCardResult.m_CheckCardReturnResult := CharRoleAdmin
        if m_TempClient.m_StrUserRole = RoleNurse then
        ...
        m_TermRecvFrameHead.m_sgType := sgClientNotRegist;
    end;
  end;
      //反馈信息至终端
  m_TermRecvFrameHead.m_skType := m_FrameHead.m_skType;
  m_TermRecvFrameHead.m_CarTermNum := m_FrameHead.m_CarTermNum;
  m_TermRecvFrameHead.m_WLenthHigh := 0;
  m_TermRecvFrameHead.m_WLenthLow := SizeOf(m_TermCheckCardResult);
  CopyMemory(@m_SendBuffer[1], @m_TermRecvFrameHead, SizeOf(m_TermRecvFrameHead));
  CopyMemory(@m_SendBuffer[1 + TermRecvFrameHeadLth], @m_TermCheckCardResult, SizeOf(m_TermCheckCardResult));
  m_CRCCheck := CalCrc(@m_SendBuffer[1], TermRecvFrameHeadLth + SizeOf(m_TermCheckCardResult));
...
begin
  ...
  begin
    ...
    begin
     //寻找病人及用户
      m_TempClient := FindClientByUserNum(DoubleToHexStr(m_TermCheckSec.m_CardUserNum));
      m_TempPatientIf := FindPatientByTermNum(inttostr(m_FrameHead.m_CarTermNum));
      if (m_TempClient <> nil) and (m_TempPatientIf <> nil) then
      begin
        try

    //连接数据库
          m_StrSql := 'UPDATE cds_call_info SET RespNum = ''' + DoubleToHexStr(m_TermCheckSec.m_CardUserNum) + ''', RespName = ''' + m_TempClient.m_StrUserName + ''', TwiceRespTime = ''' + DateTimeToStr(now()) +
...
begin
  ...
  m_CRCCheck := CalCrc(@m_SendBuffer[1], TermRecvFrameHeadLth + SizeOf(m_DrugScanResult));
...
var
  ...
  m_StrXml: TNativexml;
  m_CardCommuDtLength: Cardinal;
  m_StrInfo, m_StrSql, m_StrTempNum: string;
  m_WLengthHigh, m_WLengthLow: word;
  m_BlCheck: boolean;
  m_SendBuffer: array[1..256] of byte;
  m_CRCCheck: byte;
begin
  ...
  begin
    ...
    // 查询HIS数据库
          m_StrSql := 'select doctAdviceContent from cds_doct_advice_info where PatientNum=''' + m_TempPatientIf.m_StrPatientNum + '''';
          with TADOQuery.Create(nil) do
          begin
            try
        //连接数据库并查找记录
              ...
              try
                m_StrXml := TNativeXml.CreateName('StrOrderInquiry');
                m_StrXml.XmlFormat := xfReadable;
                m_StrXml.EncodingString   := 'UTF-8';//编码
                m_StrXml.ExternalEncoding := seUTF8;
                if not IsEmpty then
                  m_StrXml.Root.WriteString('OrderInfo', fieldbyname('doctAdviceContent').asstring)
                else
                  m_StrXml.Root.WriteString('OrderInfo', '无');

                m_StrInfo := m_StrXml.WriteToString;
  ...
  begin
    m_TermRecvFrameHead.m_WLenthHigh := 0;
    m_TermRecvFrameHead.m_WLenthLow := Length(m_StrInfo);
    CopyMemory(@m_SendBuffer[1], @m_TermRecvFrameHead, SizeOf(m_TermRecvFrameHead));
    CopyMemory(@m_SendBuffer[1 + TermRecvFrameHeadLth], @m_StrInfo[1], Length(m_StrInfo));
    m_CRCCheck := CalCrc(@m_SendBuffer[1], TermRecvFrameHeadLth + SizeOf(m_StrInfo));
    CopyMemory(@m_SendBuffer[1 + TermRecvFrameHeadLth + Length(m_StrInfo)], @m_CRCCheck, 1);
    _RPTcpServer.SendData(m_Iocpsocket, @m_SendBuffer[1], 1 +
      TermRecvFrameHeadLth + Length(m_StrInfo));
  end
  else
  begin
    m_TermRecvFrameHead.m_WLenthHigh := 0;
    m_TermRecvFrameHead.m_WLenthLow := 0;
    CopyMemory(@m_SendBuffer[1], @m_TermRecvFrameHead, SizeOf(
      m_TermRecvFrameHead));
    m_CRCCheck := CalCrc(@m_SendBuffer[1], TermRecvFrameHeadLth);
    CopyMemory(@m_SendBuffer[1 + TermRecvFrameHeadLth], @m_CRCCheck, 1);
    _RPTcpServer.SendData(m_Iocpsocket, @m_SendBuffer[1], 1 +
...
begin
  ...
  CopyMemory(@m_SendBuffer[1], @m_ClientRecvFrameHead, SizeOf(m_ClientRecvFrameHead));
  m_CRCCheck := CalCrc(@m_SendBuffer[1], ClientRecvFrameHeadLth);
...
var
  ...
  m_TempTerm: PCDSTermEmployee;
  m_CardCommuDtLength: Cardinal;
  m_SendBuffer: array[1..256] of AnsiChar;
  m_sgType: char;
  m_CRCCheck: byte;
...
begin
  ...
  begin
    ...
    begin
          //转发控制请求至终端端
      _RPTcpServer.SendData(m_Iocpsocket, pbyte(m_RecvBuf), m_BufLenth);
...
begin
  ...
  begin
    ...
    try
      for m_IntiLoop := 0 to count - 1 do
      begin
        m_TempClient := Items[m_IntiLoop];
        if m_TempClient.m_StrUserNum = inttostr(m_FrameHead.m_CarTermNum) then
        begin
...
procedure TMainFrm.CdsTermOrderExecute(m_FrameHead: TCDSTermSendCommFrameHead; m_RecvBuf: pchar; m_BufLenth: integer; m_Iocpsocket: TRPClientContext);
var
  m_TermRecvFrameHead: TCDSTermRecvCommFrameHead;
  m_TempTerm: PCDSTermEmployee;
  m_TempClient: PCDSClientEmployee;
  m_TempPatientIf: PCDSPatientInfo;
  m_InspectionTableInfo: TCDSInspectionTableInfo;
  m_TermOrderExecute: TCDSTermOrderExecute;
  m_CardCommuDtLength: Cardinal;
  m_StrSql: string;
  m_BlCheck: boolean;
  m_SendBuffer: array[1..256] of byte;
...
begin
  ...
  begin
     //读取终端查房信息
    ReadBuffer((m_RecvBuf + TermSendFrameHeadLth)^, m_TermOrderExecute, sizeOf(m_TermOrderExecute));
    m_TempTerm := FindTermBySocket(m_Iocpsocket);
    if m_TempTerm = nil then
      m_TermRecvFrameHead.m_sgType := sgTermSocketNotRegist
    else
    begin
      //寻找病人及用户
      m_TempClient := FindClientByUserNum(DoubleToHexStr(m_TermOrderExecute.m_CardUserNum));
      if m_TempClient <> nil then
      begin
        ...
        begin
          ...
          m_InspectionTableInfo.m_StrExecutorNum := DoubleToHexStr(m_TermOrderExecute.m_CardUserNum);
          m_InspectionTableInfo.m_StrExecutorName := m_TempClient.m_StrUserName;
          if m_TermOrderExecute.m_ExecuteResult = #0 then
          begin
            m_InspectionTableInfo.m_InspectionResult := '无异常';
          end
          else
            m_InspectionTableInfo.m_InspectionResult := '有异常';

          m_InspectionTableInfo.m_ExecuteTime := now(); // m_TermCheckCard.m_CheckCardTime;
    ...
    //
        ...
        //连接数据库并查找记录
          ...
          m_TermRecvFrameHead.m_sgType := sgPatientNotFound;
  ...
  m_CRCCheck := CalCrc(@m_SendBuffer[1], TermRecvFrameHeadLth);
  CopyMemory(@m_SendBuffer[1 + TermRecvFrameHeadLth], @m_CRCCheck, 1);
  _RPTcpServer.SendData(m_Iocpsocket, @m_SendBuffer[1], 1 + TermRecvFrameHeadLth);
end;
...
procedure TMainFrm.CdsTermGetPatientInfo(m_FrameHead: TCDSTermSendCommFrameHead; m_RecvBuf: pchar; m_BufLenth: integer; m_Iocpsocket: TRPClientContext);
var
  m_TermRecvFrameHead: TCDSTermRecvCommFrameHead;
  m_TempTerm: PCDSTermEmployee;
  m_TempPatientIf: PCDSPatientInfo;
  ...
  m_WLengthHigh, m_WLengthLow: word;
  m_BlCheck: boolean;
  m_SendBuffer: array[1..1024] of byte;
  m_CRCCheck: byte;
...
begin
  ...
  begin
    ...
    // 查询HIS数据库
          m_StrSql := 'select * from cds_patient_info where PatientNum=''' + m_TempPatientIf.m_StrPatientNum + '''';
          with TADOQuery.Create(nil) do
          begin
            try
        //连接数据库并查找记录
              ...
              try
                m_StrXml := TNativeXml.CreateName('StrGetPatientInfo');
                m_StrXml.XmlFormat := xfReadable;
                m_StrXml.EncodingString   := 'UTF-8';//编码
                m_StrXml.ExternalEncoding := seUTF8;
                if not IsEmpty then
                begin
               if fieldbyname('BedNum').asstring<>'' then
                  m_StrXml.Root.WriteString('BedNum', fieldbyname('BedNum').asstring)
                  else
                  m_StrXml.Root.WriteString('BedNum', '无');

                   if fieldbyname('PatientNum').asstring<>'' then
                  m_StrXml.Root.WriteString('PatientNum', fieldbyname('PatientNum').asstring)
                  else
                  m_StrXml.Root.WriteString('PatientNum', '无');

                   if fieldbyname('PatientName').asstring<>'' then
                  m_StrXml.Root.WriteString('PatientName', fieldbyname('PatientName').asstring)
                  else
                  m_StrXml.Root.WriteString('PatientName', '无');

                   if fieldbyname('PatientSex').asstring<>'' then
                  m_StrXml.Root.WriteString('PatientSex', fieldbyname('PatientSex').asstring)
                  else
                  m_StrXml.Root.WriteString('PatientSex', '无');

                  if fieldbyname('PatientAge').asstring<>'' then
                  m_StrXml.Root.WriteString('PatientAge', fieldbyname('PatientAge').asstring)
                  else
                  m_StrXml.Root.WriteString('PatientAge', '无');

                  if fieldbyname('PatientOprationInfo').asstring<>'' then
                  m_StrXml.Root.WriteString('PatientOprationInfo', fieldbyname('PatientOprationInfo').asstring)
                  else
                  m_StrXml.Root.WriteString('PatientOprationInfo', '无');

                  if fieldbyname('ChargeDoctName').asstring<>'' then
                  m_StrXml.Root.WriteString('ChargeDoctName', fieldbyname('ChargeDoctName').asstring)
                  else
                  m_StrXml.Root.WriteString('ChargeDoctName', '无');

                   if fieldbyname('NursGrade').asstring<>'' then
                  m_StrXml.Root.WriteString('NursGrade', fieldbyname('NursGrade').asstring)
                  else
                  m_StrXml.Root.WriteString('NursGrade', '无');

                  if fieldbyname('AdmissionTime').asstring<>'' then
                  m_StrXml.Root.WriteString('AdmissionTime', fieldbyname('AdmissionTime').asstring)
                  else
                  m_StrXml.Root.WriteString('AdmissionTime', '无');

                    if fieldbyname('CostInfo').asstring<>'' then
                  m_StrXml.Root.WriteString('CostInfo', fieldbyname('CostInfo').asstring)
                  else
                  m_StrXml.Root.WriteString('CostInfo', '无');

                  if fieldbyname('DiseaseInfo').asstring<>'' then
                  m_StrXml.Root.WriteString('DiseaseInfo', fieldbyname('DiseaseInfo').asstring)
                  else
                  m_StrXml.Root.WriteString('DiseaseInfo', '无');

                   if fieldbyname('AllergyInfo').asstring<>'' then
                  m_StrXml.Root.WriteString('AllergyInfo', fieldbyname('AllergyInfo').asstring)
                  else
                  m_StrXml.Root.WriteString('AllergyInfo', '无');

                    if fieldbyname('DietIdentification').asstring<>'' then
                  m_StrXml.Root.WriteString('DietIdentification', fieldbyname('DietIdentification').asstring)
                  else
                  m_StrXml.Root.WriteString('DietIdentification', '无');

                    if fieldbyname('DrugIdentification').asstring<>'' then
                  m_StrXml.Root.WriteString('DrugIdentification', fieldbyname('DrugIdentification').asstring)
                  else
                  m_StrXml.Root.WriteString('DrugIdentification', '无');

                   if fieldbyname('InOutIdentification').asstring<>'' then
                  m_StrXml.Root.WriteString('InOutIdentification', fieldbyname('InOutIdentification').asstring)
                  else
                  m_StrXml.Root.WriteString('InOutIdentification', '无');

                  if fieldbyname('MattersAttention').asstring<>'' then
                  m_StrXml.Root.WriteString('MattersAttention', fieldbyname('MattersAttention').asstring)
                  else
                  m_StrXml.Root.WriteString('MattersAttention', '无');
                end
                else
                begin
                  m_StrXml.Root.WriteString('BedNum', '无');
  ...
  begin
    ...
    CopyMemory(@m_SendBuffer[1], @m_TermRecvFrameHead, SizeOf(m_TermRecvFrameHead));
    CopyMemory(@m_SendBuffer[1 + TermRecvFrameHeadLth], @m_StrInfo[1], Length(m_StrInfo));
    m_CRCCheck := CalCrc(@m_SendBuffer[1], TermRecvFrameHeadLth + Length(m_StrInfo));
    CopyMemory(@m_SendBuffer[1 + TermRecvFrameHeadLth + Length(m_StrInfo)], @m_CRCCheck, 1);
    _RPTcpServer.SendData(m_Iocpsocket, @m_SendBuffer[1], 1 +
      TermRecvFrameHeadLth + Length(m_StrInfo));
  end
...
begin
  ...
  try
    ...
    begin
      ...
      if (GetTickCount - m_TempClient.m_CrdLastActive >
        m_ServerInfoUpdateTimer.Interval * 20) then //若10分钟未check，断开连接
      begin
        if (m_TempClient.m_IocpSocket <> nil) then
        begin
          if m_TempClient.m_IocpSocket.FSocketConn then
            m_TempClient.m_IocpSocket.CloseSocket;
  ...
  try
    ...
    begin
      ...
      if (GetTickCount - m_TempTerm.m_CrdLastActive > m_ServerInfoUpdateTimer.Interval * 20) then
      begin
        if (m_TempTerm.m_IocpSocket <> nil) then
        begin
          if m_TempTerm.m_IocpSocket.FSocketConn then
            m_TempTerm.m_IocpSocket.CloseSocket;
